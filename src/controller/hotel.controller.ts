...
import { getHotelsByService, getHotelsByRating, getHotelsByRateAvg } from "../utils/hotel";
import { getListRoomActive, getQuantityRoomsIsActive } from "../utils/room";
...
const HotelController = {
	async getDetailHotel(req: Request, res: Response) {
		try {
			...
			const hotelService = { ...hotelRating, services: services.map(service => service.serviceName), formsFeedback: resultForms };
	...
	async getHotelBySearch(req: Request, res: Response): Promise<any> {
		try {
			...
			const resultSearchHotel = listHotelByCity.filter((hotel, index) => hotelsMapBoolean[index]).map(hotel => hotel.toJSON());
			if (resultSearchHotel.length === 0) {
				return res.status(200).json([]);
			}
			const hotelService = await getHotelsByService(resultSearchHotel);
			const hotelRating = await Promise.all(hotelService.map(async(hotel: any) => await getHotelsByRateAvg(hotel)))
	...
	async getHotelByFilter(req: Request, res: Response) {
		try {
			...
			const filterHotel = hotelsMapBoolean.filter((hotel: any, index) => hotel.isAvailable) as any;
			if (filterHotel.length === 0) {
				...
				const ResultFilterByService = await getHotelsByService(hotelAvailable);
				const hotelRating = await Promise.all(ResultFilterByService.map(async(hotel: any) => await getHotelsByRateAvg(hotel)))
				return res.status(200).json(hotelRating)
			}
		}
		catch (error) {
